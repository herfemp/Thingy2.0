...
// D/C = Data Pin high, Command pin low
void static writecommand(uint8_t c) {
	SETBIT(((GPIO_TypeDef *) (GPIOB_BASE))->BRR,  0x0002);
	SETBIT(((GPIO_TypeDef *) (GPIOA_BASE))->BRR,  0x0002);

	SPI1->DR = c;
	while(!(SPI1->SR & SPI_I2S_FLAG_RXNE));
	c =  SPI1->DR;

	SETBIT(((GPIO_TypeDef *) (GPIOA_BASE))->BSRR,  0x0002);
}


void static writedata(uint8_t c) {
	SETBIT(((GPIO_TypeDef *) (GPIOB_BASE))->BSRR,  0x0002);
	SETBIT(((GPIO_TypeDef *) (GPIOA_BASE))->BRR,  0x0002);
...
void Delay1ms(uint32_t n){uint32_t volatile time;
  while(n){
    time = 90000*2/91;  // 1msec, tuned at 80 MHz
    while(time){
      time--;
    }
    n--;
...
// BackgroundStTextColor color is fixed at black
